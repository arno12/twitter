...
if __name__ == '__main__':
    ...
    for company in companies:
        ...
        locs = [[tweet.id, tweet.metadata['iso_language_code'], tweet.created_at, tweet.user.screen_name, tweet.text,
                 tweet.user.location, tweet.favorite_count, tweet.retweet_count, datetime.now()] for tweet in tweets]
	...
	Path('./logs').mkdir(parents=True, exist_ok=True) 	
        ...
        df = pd.DataFrame(
            data=locs,
            columns=["id", "iso_language_code", "created_at", "screen_name", "text", "location", "favorite_count",
                     "retweet_count", "queried_at"])

        # Load previous data if it exists
        last_results_path = Path("./results/{}_searches_incremental.csv".format(company))
